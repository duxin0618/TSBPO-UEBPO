...
class SCQDMBPO(RLAlgorithm):
    ...
    def __init__(
        ...
        self.ratios_threshold = ratios_threshold
        self._scheme = scheme
        self.reward_weight=reward_weight
        self.use_reward_weight=use_reward_weight
        self.count13 = []
        self.count10 = []
        self.number_ratios = []
    ...
    def _train(self):
        ...
        if self._use_src:
            ...
            info = "plan {}, fix iters: {}, beta_filter: {}, difference_threshold: {}".format(self._plan, max_iter, self.beta_filter, self.difference_threshold)
            if self.use_reward_weight:
                info = info+", ratios_threshold: {}, reward_weight: {}".format(self.ratios_threshold, self.reward_weight)
                if self._scheme == 0:
                    f_log.write("scheme: optimistic\n")
                elif self._scheme == 1:
                    f_log.write("scheme: conservative\n")
            info = info + "\n"
            print(info)
            f_log.write(info)
            f_log.close()
            ...
            info = "plan {}\n".format(
                self._plan)
            if self.use_reward_weight:
                info = info + ", ratios_threshold: {}, reward_weight: {}".format(self.ratios_threshold,
                                                                                 self.reward_weight)
                if self._scheme == 0:
                    f_log.write("scheme: optimistic\n")
                elif self._scheme == 1:
        ...
        self.pred_diagnostics = OrderedDict()
        ...
        for self._epoch in range(self._epoch, self._n_epochs):
                ...
                if self._timestep % self._model_train_freq == 0:  # 250
                    ...
                    if self._use_src and self._src_epoch_stop > self._epoch \
                        ...
                        model_by_src_function_train_metrics = self._train_model_by_src_function(batch_size=256,
                                                                                                ...
                                                                                                holdout_ratio=0.2,
                                                                                                max_t=self._max_model_t)
                        model_metrics = self.constructor_model_metrics(model_by_src_function_train_metrics, model_metrics)
            ...
            diagnostics.update(OrderedDict((
                ...
                *(
                    (f'model/{key}', self.pred_diagnostics[key])
                    for key in sorted(self.pred_diagnostics.keys())
                ),
                ('sample_uncertainty_number ', self.sample_uncertainty_number),
                ('sample_uncertainty_number_bonus ', self.sample_uncertainty_number_bonus),
                ('number_greater_than_1.3', str(self.count13)),
                ('number_greater_than_1.0', str(self.count10)),
                ('number_of_samples', str(self.number_ratios)),
    ...
    def _save_true_samples(self, **kwargs):

        start_sample, trajs = self.sampler.sample_trajectory(20000)
        # print("true traj number is: ", len(start_sample))
        # print("true samples number is: ", len(trajs['observations']))

        # all_true_samples = self._pool.return_all_samples()
        file_name = self.samples_dir+'/true_samples_'+str(self._epoch)+'_'+str(self._tag)+'.pkl'
        with open(file_name, 'wb') as pkl_file:
            pickle.dump(trajs, pkl_file)
        self._save_fake_samples(start_sample, **kwargs)

    def _save_fake_samples(self, start_sample, **kwargs):
        trajs = self._rollout_fake_samples(start_sample, **kwargs)
        # all_fake_samples = self._model_pool.return_all_samples()
        file_name = self.samples_dir+'/fake_samples_' + str(self._epoch) + '_' +str(self._tag)+ '.pkl'
        with open(file_name, 'wb') as pkl_file:
            pickle.dump(trajs, pkl_file)
    ...
    def _rollout_model(self, rollout_batch_size, **kwargs):
        ...
        for j in range(bh):
            ...
            for i in range(self._rollout_length):
                ...
                std = info["std"]
                # print("std: ", std.shape)
                ensemble_std = info["ensemble_std"]
                # print("ensemble_std: ", ensemble_std.shape)

                ensemble_obs_std = ensemble_std[:, :, 1:]
                obs_std = std[:, 2:]
                obs_std = np.linalg.norm(obs_std, axis=-1)
                ensemble_obs_std = np.linalg.norm(ensemble_obs_std, axis=-1).mean(0)
                self.record_tabular_compute("obs_uncertainty_std_", obs_std, self.pred_diagnostics)
                self.record_tabular_compute("ensemble_obs_uncertainty_std_", ensemble_obs_std, self.pred_diagnostics)
                self.sample_uncertainty_number = ensemble_obs_std.shape[0]
                self.sample_uncertainty_number_bonus =  obs_std.shape[0]

                std_data_new = {"ensemble_obs_std":ensemble_obs_std, "obs_std":obs_std}

                if self._epoch % 25 == 0 and j == 0:
                    std_data_path = os.path.join(os.path.dirname(self.diagnostics_file), self._env_name+"_"+str(self._epoch)+"_"+str(i)+"_std.npy")
                    if os.path.exists(std_data_path):
                        aux = np.load(std_data_path, allow_pickle=True).item()
                        aux_ensemble_std, aux_std = aux['ensemble_obs_std'], aux['obs_std']
                        aux_ensemble_std = np.concatenate((aux_ensemble_std, ensemble_obs_std), axis=0)
                        aux_std = np.concatenate((aux_std, obs_std), axis=0)
                        std_data_new = {"ensemble_obs_std": aux_ensemble_std, "obs_std": aux_std}

                    np.save(std_data_path, std_data_new)

                if self.use_reward_weight:
                    ratios = obs_std / ensemble_obs_std * 1.0 + 0.0000001
                    if j == 0:
                        self.count13.append(np.sum(ratios > 1.3))
                        self.count10.append(np.sum(ratios > 1.0))
                        self.number_ratios.append(np.size(ratios))
                    if self._scheme == 0:  # optimistic
                        ratios[ratios > self.ratios_threshold] = self.reward_weight
                        ratios[ratios <= self.ratios_threshold] = 1

                        rew = rew * ratios[:, np.newaxis]
                    elif self._scheme == 1:  # conservative
                        ratios[ratios > self.ratios_threshold] = self.reward_weight
                        ratios[ratios <= self.ratios_threshold] = 1
    ...
    def _rollout_by_src_function(self, rollout_batch_size, **kwargs):
        ...
        for j in range(rollout_src_length):
            ...
            if j > 0:
                ...
                for i in range(ins):
                    ...
                    # R-Q discriminator
                    # R function
                    real_rwd = self._static_fns.reward_fn(real_cur_obs[start_index: end_index], cur_act[start_index: end_index], real_cur_obs_next[start_index: end_index])
                    fake_rwd = self._static_fns.reward_fn(cur_obs[start_index: end_index], cur_act[start_index: end_index], cur_next_obs[start_index: end_index])
                    # Q function
                    ...
                    samples = {'observations': cur_obs[start_index: end_index][samples_index], 'actions': cur_act[start_index: end_index][samples_index], 'next_observations': cur_next_obs[start_index: end_index][samples_index],
        ...
        print(
            '[ By SCQD function Model Rollout ] Added: {:.1e} | SC Model pool: {:.1e} (max {:.1e}) | Length: {} | Train rep: {}'.format(
        ...
        for j in range(rollout_src_length):
            ...
            if j > 0:
                ...
                for i in range(ins):
                    ...
                    samples = {'observations': cur_obs[start_index: end_index], 'actions': cur_act[start_index: end_index], 'next_observations': cur_next_obs[start_index: end_index],
    ...
    def record_tabular_compute(self, key, values, record_tabular, placement='back'):
        if placement == 'front':
            prefix = ""
            suffix = key
        else:
